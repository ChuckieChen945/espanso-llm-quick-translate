...
async def generate_tts_audio(text: str, filename: str = "translated.mp3",sound_name:str="en-GB-LibbyNeural"):
...
def change_color(text: str) -> str:
    if text:
        return (
            text.replace('<green>', '<green_t>').replace('</green>', '</green_t>')
                .replace('<red>', '<red_t>').replace('</red>', '</red_t>')
                .replace('<yellow>', '<yellow_t>').replace('</yellow>', '</yellow_t>')
        )
    return None
...
def background_worker(text: str, result: str, audio_file: str, diff_file: str, sound_name: str, auto_play: bool):
...
def main():
    if len(sys.argv) != 7:
        print("❌ Usage: python background_worker.py <text> <result> <audio_file> <diff_file> <sound_name> <auto_play>")
        sys.exit(1)
